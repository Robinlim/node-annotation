...
function delegate(instance, _methods, method) {
    ...
    instance[method] = function() {
        ...
        if (_methods[method] && (callback = _methods[method][PROXYCYCLE.THROWS])) {
            ...
            wrapFunct = function(){
                return oriFunct.apply(self, args);
            }
        }
        //around instance run
        if (_methods[method] && (callback = _methods[method][PROXYCYCLE.AROUND])) {
            result = decorateCallbacks(callback, function() {
                return wrapFunct();
            }, args)();
        } else {
            //instance run
...
function decorateCallbacks(cbs, callback, args) {
    var next = callback;
    _.forEach(cbs, function(cb, i) {
        next = decorateCallback(cb, next, args);
    });
    return next;
}

function decorateCallback(cb, next, args) {
